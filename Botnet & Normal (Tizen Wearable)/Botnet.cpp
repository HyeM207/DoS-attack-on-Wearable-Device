...
int main(int argc, char *argv[])
{
   ...
   //            int runtime[5] = {600, 600, 600, 600, 1200};
			   int runtime[5] = {60, 60, 60, 60, 120};
			   int abnormal_round_sec[5] = {1, 1, 1, 1, 1};
			   int abnormal_packet_num[5] = {80, 50, 70, 50, 80};
			   ...
			   for (int i = 0; i < 5; i++){
				...
				while(1){
					...
					if ((i == 1 || i == 3) && (num == 1 || num % 1500 == 0)){
						dlog_print(DLOG_DEBUG, LOG_TAG, "[default connection] send to victim (normal)");
						toVictim(victim_ip, victim_port, num);
						cnt++;
						dlog_print(DLOG_DEBUG, LOG_TAG, "|---------[ Send normal Cnt %d ]------|", cnt);
						num++;
					}

					gettimeofday(&round_tv, NULL);
					round_begin = (round_tv.tv_sec) * 1000 + (round_tv.tv_usec) / 1000 ;
					for (int p = 0; p < abnormal_packet_num[i]; p++){
						gettimeofday(&tv, NULL);
						end = (tv.tv_sec) * 1000 + (tv.tv_usec) / 1000 ;
						if(((end - begin) / 1000) >= runtime[i]){
						   break;
					...
					while(1){
						...
						if(((round_end - round_begin) / 1000) >= abnormal_round_sec[i]){
						   break;
						}
					}
				}
